...
#filter basic_table for micronutirent contents which are calcium, folate, magnesium, potassium, sodium 
queries <- c("American", "French", "Main Dishes", "390^Pescetarian", "397^Egg-Free")
a <- generate_param(queries)
basic_table <- create_table(a)

cuisine_table <- basic_table

#assign values to cuisine 1 and cuisine 2 
micro_table2 <- process_basic_table(cuisine_table, "American", "French")
cuisine1 <- "American"
cuisine2 <- "French"
